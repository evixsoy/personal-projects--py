...
def seznamknih():
    ...
    return ids,kniha,autor,rok,zanr,pocetkusu,contents
...
def vyhledatknihu(ids,kniha,autor,rok,zanr,pocetkusu,category,temp_value):
    krok = 0
    not_found = 0
    for i in category:
        if i == temp_value:
...
def upravit_polozku(contents):
    print("Napište ID polozky")
    valueinput = int(input(">"))
    #TODO err handling kdyz zada vetsi cislo jak len ids
    print("Název Knihy:")
    nove_kniha = input(">")
    print("Název Autora:")
    novy_autor = input(">")
    ...
    temp_contentslist = []
    for i in contents:
        temp_contentslist.append(i)
    temp_contentslist.pop(valueinput-1)
    polozka_new = f"{valueinput};{nove_kniha};{novy_autor};{novy_rok};{novy_zanr};{nove_pocet}"
    temp_contentslist.insert(valueinput-1, polozka_new)
    print(temp_contentslist)
    ...
    #TODO nacist zpatky temp_contentslist do books.txt

def chod_programu():
    valueinput = 1
    ids,kniha,autor,rok,zanr,pocetkusu,contents= seznamknih()
    while valueinput != 0:
        valueinput = menu()
        match valueinput:
            case 1:
            ...
            case 2:
                ...
                match valueinput:
                    ...
                    case _:
                        ...
                        pass
                if temp_value != None:
                    print(vyhledatknihu(ids,kniha,autor,rok,zanr,pocetkusu,category,temp_value))
            case 3:
                pridat_polozku(ids)
            case 4:
                upravit_polozku(kniha,autor,rok,zanr,pocetkusu,contents)
chod_programu()
